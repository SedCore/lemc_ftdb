...
def add_gradient_penalty(model, lambda_coeff=0.5, penalty_type="two-sided"):
    term = K.gradients(tf.keras.ops.sum(model.output, axis=1), model.input)
    term = tf.keras.ops.square(term)

    if penalty_type == "two-sided":
        penalty = (term - 1) ** 2
    elif penalty_type == "one-sided":
        penalty = tf.keras.ops.max(0, term - 1)
    else:
    ...
    penalty = K.in_train_phase(penalty, tf.keras.ops.zeros(shape=(1,)))

    model.add_loss(penalty)


def add_l2_regularization(model, l2_strength=1e-4):
    # For Functional models, we need to collect all regularization losses
    reg_losses = []
    for layer in model.layers:
        for tw in layer.trainable_weights:
            reg_losses.append(tf.keras.regularizers.l2(l2_strength)(tw))
    
    # Add the regularization losses to the model
    if reg_losses:
        # For Functional models, we add regularization via the layers
        for layer in model.layers:
            if hasattr(layer, 'kernel_regularizer') and layer.kernel_regularizer is None:
                layer.kernel_regularizer = tf.keras.regularizers.l2(l2_strength)
...
class RBFClassifier(Layer):
    ...
    def build(self, input_shape):
        ...
        # Always use a valid initializer string or object
        centroid_init = self.centroids_initializer if self.trainable_centroids else "zeros"

        self.centroids = self.add_weight(
            name="centroids",
            shape=(self.centroid_dims, self.num_classes),
            dtype="float32",
            trainable=self.trainable_centroids,
            initializer=centroid_init
        )
        self.kernels = self.add_weight(
            name="kernels",
            shape=(self.centroid_dims, self.num_classes, in_features),
    ...
    def call(self, inputs, training=None):
        z = tf.keras.ops.einsum("ij,mnj->imn", inputs, self.kernels)
        out = self.rbf(z)
    ...
    def rbf(self, z):
        z = z - self.centroids
        z = tf.keras.ops.mean(tf.keras.ops.square(z), axis=1) / (2.0 * self.length_scale ** 2)
        z = tf.keras.ops.exp(-z)
    ...
    def update_centroids(self, inputs, targets):
        ...
        if isinstance(self.centroids, tf.Variable):
            self.centroids.assign(self.m / self.n)
