...
class SpectralNormalization(layers.Wrapper):
  ...
  def __init__(self,
    ...
    if not isinstance(layer, layers.Layer):
      raise ValueError('`layer` must be a `tf.keras.layer.Layer`. Observed `{}`'.format(layer))
    super().__init__(
  ...
  def build(self, input_shape):  # pytype: disable=signature-mismatch  # overriding-parameter-count-checks
    ...
    # Try self.layer.kernel, if not available, do self.layer.depthwise_kernel.
    try:
      _ = self.layer.kernel
    except AttributeError:
      try:
        self.layer.kernel = self.layer.depthwise_kernel
      except AttributeError:
        raise ValueError('`layer` must contain `kernel` or `depthwise_kernel` attribute. Observed `{}`'.format(self.layer))
...
@register_keras_serializable()
class SpectralNormalizationConv2D(layers.Wrapper):
