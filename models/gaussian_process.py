...
class RandomFeatureGaussianProcess(tf.keras.layers.Layer):
  ...
  def call(self, inputs, global_step=None, training=None):
    ...
    if self.return_gp_cov:
      gp_covmat = self._gp_cov_layer(gp_feature, logits=gp_output, training=training)
...
class LaplaceRandomFeatureCovariance(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.precision_matrix = (
        self.add_weight(
            ...
            trainable=False,
            aggregation='only_first_replica'))#tf.VariableAggregation.ONLY_FIRST_REPLICA))
    self.built = True
  ...
  def _get_training_value(self, training=None):
    if training is None:
      training = False#tf.keras.backend.learning_phase()
